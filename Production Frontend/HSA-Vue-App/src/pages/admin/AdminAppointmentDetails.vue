<template>
   ...
   <section v-if="selectedAppointment && !isLoading">
    ...
    <base-button-container>
          <base-button @click="submit('approved')" mode="primary" 
            >Approve</base-button
          > 
          <base-button @click="submit('cancelled')" mode="red" 
            >Cancel</base-button>
          <base-button @click="back" mode="flat" 
            >Go Back</base-button
          >
    </base-button-container>
  </section>
...
//     },
    checkForm() {
      ...
      if (
        !this.time ||
        this.time === "" ||
        !/^([01]\d|2[0-3]):([0-5]\d)$/.test(this.time)
      ) {
        this.errors.push("booking time is invalid");
      }
    ...
    computed:{
      ...
      selectedAppointment() {
        return this.$store.getters["admin/appointmentDetails"];
      },
      getHospitalAddress(){
    ...
    return labTestString;
      ...
      isFeesExist(){
        return this.selectedAppointment?.fees?.length > 0 ?? false;
      },
      isNotesExist(){
