<template>
 ...
 <div v-if="isLoading && !error">
    ...
    <section v-if="!shift">
      <h2>No current appointment for this doctor yet.</h2>
        </section>
     <section v-if="shift" class="appointment-row-container">
    <h1> Appointments</h1>
    <h4>Date:{{ getDate }} Shift:{{getShift  }}</h4>
...
export default {
  ...
  computed:{ isLoggedIn() {
    ...
    shift(){
          return this.$store.getters['admin/selectedDoctorTodayAppointments']?.length??0;
       },
    // isCoach() {
    //   return this.$store.getters
    // },
    appointments(){
      const appointments= this.$store.getters['admin/selectedDoctorTodayAppointments'];
...
for (const appointment of appointments) {
  ...
  return result;
    ...
    getShift(){
  const shift = this.$store.getters['admin/selectedDoctorTodaySchedule'].shift;
  if(!shift || shift.startTime || shift.endTime){
    ...
    getDate(){
      const date = this.$store.getters['admin/selectedDoctorTodaySchedule']?.date;
      if(!date) return null;
